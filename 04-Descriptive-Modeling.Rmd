...
We can also use hexagone BAUs using
...
Note that alghouth spatio-temporal basis functions are generated from tensor product, resulting S-T covariance function is not separable. \
...
## Lab: Temporal Basis Functions with SpatioTemporal package

In this lab, we fit the maximum temperature dataset using temporal basis functions. We will use the $\texttt{SpatioTemporal}$ package. We assume following model

\begin{equation}
  Y(s;t) = x(s;t)' \beta + \sum^{n_\alpha}_{i=1} \phi_i(t) \alpha_i(s) + \nu(s;t) \tag{21}
\end{equation}
